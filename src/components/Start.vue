	...
	<div class="app-container">
		...
		<div class="content-container">
			...
			<div v-else-if="currentStep === 'start'" class="start-survey-container">
				<h2>Bonjour,<br> pour mieux connaître les usagers<br> de la gare de Lorient,<br><br> Lorient Agglomération souhaiterait en savoir plus sur votre déplacement en cours.<br> Auriez-vous quelques secondes à nous accorder ?
				</h2>
			...
			<div v-else-if="currentStep === 'survey' && !isSurveyComplete">
				<div class="question-container">
					...
					<button v-if="['Q3a', 'Q3a_nv'].includes(currentQuestion.id)" @click="showPdf = true"
						class="btn-pdf">
						Voir le plan du parking
					</button>

					<!-- Commune Selector for Q2 -->
					<div v-if="currentQuestion.id === 'Q2' || currentQuestion.id === 'Q2_nv'">
					...
					<div
						v-else-if="currentQuestion.id === 'Q2_precision' || currentQuestion.id === 'Q2_precis_nv'">
...
const selectAnswer = (option, index) => {
	if (currentQuestion.value) {
		...
		if (currentQuestion.value.id === 'Q2' || currentQuestion.value.id === 'Q2_nv') {
			if (index === 0) {
				// "Lorient" selected
				const questionPrefix = currentQuestion.value.id === 'Q2' ? 'Q2' : 'Q2_nv';
				answers.value[`Q2_COMMUNE`] = 'LORIENT';
				answers.value['CODE_INSEE'] = '56121'; // INSEE code for Lorient
...
const getNextId = async () => {
	...
	return `Lorient-${counter.toString().padStart(6, '0')}`;
};
